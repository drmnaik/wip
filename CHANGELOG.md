...
- **Foundation & Scanner (Phase 0+1):** Config management (TOML read/write, interactive init), repo discovery, git status scanning (branch, dirty, stash, ahead/behind, recent branches/commits), Rich terminal display, CLI wiring (`wip`, `wip status`, `wip config`, `--json`, `--verbose`)
...
- **LLM Integration (Phase 4):** Provider abstraction (Anthropic implemented, OpenAI/Gemini stubs), `wip ai briefing`, `wip ai standup`, `wip ai ask` with streaming, prompt assembly from status data, config-driven provider/model selection
